...
- **hunches** → `vault/hunches/` → `/hunches/<slug>`
  - `slug` `title` `description?` `date?` `lastmod?` `tags?[]` `aliases?[]`
...
- For published projects/questions/hunches, keep all three names with separate jobs:
...
- `slug` is source-only frontmatter for projects/questions/hunches. Sync consumes it to choose the generated content path, then omits it from generated Astro content.
...
- Resolve by slug, then `aliases`, then collection path (e.g. `[[hunches/foo]]`).
- Folder paths supported; backlinks include projects/questions/hunches/logs/pages. Legacy `[[notes/foo]]`, `[[traces/foo]]`, and `[[guesses/foo]]` paths resolve to hunches.
