...
import './styles.css';
import { readRoute, studyLink as link, homeLink } from './guide/routes';
...
function Index({ age }) {
  return <div className="index-page">
    <header className="index-nav"><a href={homeLink({ age })} className="wordmark">small beginnings<span className="logo-dot">•</span></a><span>Montessori, by age</span></header>
    <main id="main" tabIndex={-1}>
      ...
      <section className="concept-grid" aria-label="Choose a design direction">{concepts.map((c,i) => <a key={c.id} className={`concept-card theme-${c.id}`} href={link(c.id, age)}>
...
function Footer() { return <footer className="site-footer"><span>Small beginnings</span><p>An independent companion to the books by Simone Davies and Junnifa Uzodike.<br/>Original summaries and illustrations. Follow your child’s pace.</p><a href={link('studies', readRoute().age)}>All directions ↑</a></footer>; }
...
function Study({ concept:c, age }) {
  ...
  return <div className={`study theme-${c.id}`}>
    <header className="study-nav"><a className="back-link" href={link('studies', age)}>← <span>All directions</span></a><a className="wordmark" href={homeLink({ age })}>small beginnings<span className="logo-dot">•</span></a><label className="concept-switch"><span className="sr-only">Design direction</span><select value={c.id} onChange={e=>location.hash=link(e.target.value,age)}>{concepts.map((x,i)=><option key={x.id} value={x.id}>{num(i)} · {x.name}</option>)}</select></label></header>
...
export default function LegacyStudies({ route }) {
  const concept = concepts.find(item => item.id === route.id);
  return route.id === 'studies' ? <Index age={route.age}/> : concept ? <Study key={`${concept.id}-${route.age}`} concept={concept} age={route.age}/> : null;
}
